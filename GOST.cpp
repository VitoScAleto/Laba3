...
int build_new_from_old(vector<int> prime, int bit) 
{
    ...
    while (true) 
    {
        ...
        double n = (pow(2, bit - 1) / q) + (pow(2, bit - 1) * zakaruchka / q);// пункт 1

        if ((int)n % 2 == 1) n++;

        for (int u = 0; true; u += 2) // пункт 2
        {
            p = (n + u) * q + 1;//пункт 3
            if (p > pow(2, bit)) break;// пункт 4

            if (power_mod(2, p - 1, p) == 1 && power_mod(2, n + u, p) != 1) return p;//пункт 5
        }
    }
}
